...
public class AudioObject : RegisteredComponent
{
    ...
    public void Stop( float fadeOutLength, float startToFadeTime )
    {
        ...
        if ( fadeOutLength > 0 || startToFadeTime > 0 )
        {
            if ( isFadingOut )
            {
                double requestedEndOfFadeout = AudioController.systemTime + startToFadeTime + fadeOutLength;
                ...
                else
                {
                    ...
                    double alreadyFadedTime = AudioController.systemTime - _fadeOutStartTime;
                    double timeToFinishFade = startToFadeTime + fadeOutLength;

                    double currentTimeToFinishFade = currentEndOfFadeout - AudioController.systemTime;

                    double newFadedTime;

                    newFadedTime = alreadyFadedTime * timeToFinishFade / currentTimeToFinishFade;

                    _fadeOutStartTime = AudioController.systemTime - newFadedTime;
